...
def parse_args_and_create_config():
    ...
    parser.add_argument("--wiki_topk", type=int, default=10, 
                       help="Number of top wiki search results to retrieve")
    ...
    parser.add_argument("--parallel_on_dataset", type=int, default=5, 
                       help="Number of parallel processes across datasets")
    
    # Round Configuration
    parser.add_argument("--round", type=int, default=0, 
                       help="Total number of reasoning rounds")
    ...
    infer_kwargs = {
        ...
        "parallel_on_dataset": args.parallel_on_dataset,
...
def get_search_results_with_format(task, response, history, **kwargs):
    try:
        ...
        search_results = ""
        wiki_topk = kwargs.get("wiki_topk", 5)
            ...
            for tool in parsed_content:
                if tool['type'] == 'web_search':
                    ...
                    for _ in range(3):
                        ...
                        if "BudgetExceededError" in str(web_results) or "An error occurred:" in str(web_results) or "400 Bad Request" in str(web_results):
                    ...
                    for _ in range(3):
                        ...
                        if "BudgetExceededError" in str(crawl_results) or "An error occurred:" in str(crawl_results) or "400 Bad Request" in str(crawl_results):
...
def process_single_data(query, **kwargs):
    ...
    if kwargs["adaptive"] == "auto":
        ...
        while attempt < 10:
            ...
            tag, content = test_api_client(system_prompt, query, "", "", URL, KEY, MODEL, stop_words=stop_words, **kwargs)
            current_cls = content
            _, _, _, classification_content = extract_specific_tag_parallel_toolcalldict(content, allowed_tags=["classification"])
            logging.info(f"Classification completed: {MODEL}, classification result: {classification_content}")
            if tag == "error" or content is None or classification_content not in ["toolcalling_agent", "reasoning_agent", "default_agent"]:
    ...
    elif kwargs["adaptive"] == "default_agent":
    ...
    while attempt < retry_attempts and error_count < 10:
        ...
        item_type, content = test_api_client(system_prompt, query, current_cls, current_answer, URL, KEY, MODEL, **kwargs)
        logging.info(f"Current trajectory generation completed: {MODEL}, current round response length: {count_tokens(content, tokenizer)}")
        ...
        if item_type == "error" or content is None:
            ...
            is_success, tool_calls_obs = get_search_results_with_format(query, content, current_answer_with_current_content, **kwargs)
...
def process_queries(infile, outfile, q_key, a_key, **kwargs):
    ...
    if sample_total is not None and isinstance(sample_total, int) and sample_total > 0:
        ...
        if need_sample > 0:
            ...
            logging.info(f"Need to sample {need_sample} new data, actually sampled {actual_sample}, total sample size will reach {processed_count + actual_sample}")
        else:
            # Processed count already meets or exceeds total sample requirement
            questions_data = []
            logging.info(f"Processed data count {processed_count} already meets or exceeds total sample requirement {sample_total}, no new data sampling needed")
    else:
        # If no sampling needed, keep the original shuffle operation
        random.shuffle(questions_data)
        logging.info(f"No valid sampling parameter specified, using all filtered data, total {len(questions_data)} items")
    ...
    logging.info(f"Processing complete! Success: {stats['success']}, Failed: {stats['failed']}, Total: {len(new_questions_data)}")
    return outfile
...
def main():
    ...
    # Define configurations for all datasets - each element in the list is parameters for one dataset
    datasets = [
        {
    ...
    for dataset in datasets:
        filename = dataset["infile"].split("/")[-1].replace(".json", "").replace(".jsonl", "")
        dataset["outfile_base"] = args.output_file

    # Record total start time
    total_start_time = time.time()
    # Process datasets in parallel, maximum 5 threads running simultaneously (can be adjusted as needed)
    with ThreadPoolExecutor(max_workers=INFER_KWARGS["parallel_on_dataset"]) as executor:
        # Submit all tasks
        futures = [
            executor.submit(
        ...
        for future in as_completed(futures):
            try:
                dataset_path, cost_time = future.result()
    ...
    logging.info(f"All datasets processing complete, total time taken: {total_cost_time:.2f} seconds")
    return 
